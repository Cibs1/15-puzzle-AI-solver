...
public class IDFS {
    public static void main(String[] args) throws IOException{
        ...
        while (path.isEmpty() == true){
            path=idfs(configI, initial , configF,counter);
            counter+=1;
        }
    ...
    static List<int[]> idfs(int[] state, List<int[]> visited, int[] finalState, int depth) {
        if (!to_STD(state)) {
        ...
        if ((newState = iteration_up(state)) != null && !visited.contains(newState)) {
            ...
            List<int[]> result = idfs(newState, newVisited, finalState, depth - 1);
            if (!result.isEmpty()) {
                return result;
            }
        }
    ...
    public static int calculateManhattanDistance(int[] puzzle, int[] finalConfig) {
        ...
        for (int i = 0; i < puzzle.length; i++) {
            if (puzzle[i] != 0) {
                targetRow = (finalConfig[puzzle[i] - 1] - 1) / 4; // Assuming a 4x4 board
